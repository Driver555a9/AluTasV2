...
#include "common/RaceProgressState.h"
...
namespace 
{
    ...
    void GhostRenderLayer::OnRender() noexcept
    {
        ...
        if (! s_race_against_replay || ! s_replay_to_race_against.has_value() || s_replay_to_race_against->GetAmountFrames() == 0)
        {
            ...
            {
                {
                    std::optional<ComDllOut::DllStateOut> state_out   = AsphaltDllManager::GetDllStateOutCopy();
                    camera_state_now = CameraState(state_out->m_camera_state);
                    racer_state      = RacerState(state_out->m_racer_state);
                }

                s_ghost_model->SetPosition(racer_state.GetPositionOpenGL_XYZ());
                s_ghost_model->SetRotation(racer_state.GetRotationOpenGL_XYZ());
                m_camera.SetPosition(camera_state_now.GetPositionOpenGL_XYZ());
                m_camera.SetRotation(camera_state_now.GetRotationOpenGL_WXYZ());
    ...
    void GhostRenderLayer::SetCurrentReplay(const std::optional<Replay>& replay) noexcept
    {
        ...
        s_replay_ghost_racing_line = [&replay]() -> std::optional<std::vector<CoreEngine::DrawLines3D_RenderPipeline::LineVertex>> {
            ...
            {
                glm::vec3 p0 = RacerState(frames[i].m_recorded_racer_state).GetPositionOpenGL_XYZ();
                glm::vec3 p1 = RacerState(frames[i+1].m_recorded_racer_state).GetPositionOpenGL_XYZ();
