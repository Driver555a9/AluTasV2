...
namespace AsphaltTas
{
    ...
    namespace InputDataCache
    {
        static std::mutex MUTEX;

        static libmem::Address ORIGINAL_CODE_ADDRESS = INVALID_ADDRESS;
        constexpr static size_t ORIGINAL_CODE_SIZE = 17;
        static std::array<uint8_t, ORIGINAL_CODE_SIZE> ORIGINAL_CODE;
        static bool ORIGINAL_CODE_CACHE_HAS_VALUE = false;
        ...
        constexpr static size_t ALLOCATED_DATA_SIZE = 16;
        static libmem::Address ALLOCATED_DATA_ADDRESS = INVALID_ADDRESS;
    }

    uintptr_t MemoryAddressFinder::FindInputDataBaseAddress()
    {
        std::scoped_lock lock(InputDataCache::MUTEX);
        auto [process, module] = MemoryUtility::GetAsphaltProcessAndModuleOrThrow();

        if (InputDataCache::ORIGINAL_CODE_ADDRESS == INVALID_ADDRESS)
        {
            InputDataCache::ORIGINAL_CODE_ADDRESS = MemoryUtility::AOBScanOrThrow(&process, "85 C0 0F 85 ?? ?? ?? ?? 4C 8B 4B 20 44 0F B6 45 FD", module.base, module.size);
        }

        if (!InputDataCache::ORIGINAL_CODE_CACHE_HAS_VALUE)
        {
            MemoryUtility::ReadMemoryOrThrow(&process, InputDataCache::ORIGINAL_CODE_ADDRESS, InputDataCache::ORIGINAL_CODE.data(), InputDataCache::ORIGINAL_CODE_SIZE);
            InputDataCache::ORIGINAL_CODE_CACHE_HAS_VALUE = true;
        }

        if (InputDataCache::ALLOCATED_CAVE_ADDRESS == INVALID_ADDRESS)
        {
            InputDataCache::ALLOCATED_CAVE_ADDRESS = MemoryUtility::AllocMemoryOrThrow(&process, InputDataCache::ALLOCATED_CAVE_SIZE, libmem::Prot::XRW);
        }

        if (InputDataCache::ALLOCATED_DATA_ADDRESS == INVALID_ADDRESS)
        {
            InputDataCache::ALLOCATED_DATA_ADDRESS = MemoryUtility::AllocMemoryOrThrow(&process, InputDataCache::ALLOCATED_DATA_SIZE, libmem::Prot::RW);
            // Clear buffer on first allocation
            uint8_t zero_buf[16] = { 0 };
            MemoryUtility::WriteMemoryOrThrow(&process, InputDataCache::ALLOCATED_DATA_ADDRESS, zero_buf, sizeof(zero_buf));
        }

        // Only install if not already running
        if (!InputDataCache::HOOK_INSTALLED)
        {
            try
            {
                ...
                int32_t jne_rel_offset;
                std::memcpy(&jne_rel_offset, &InputDataCache::ORIGINAL_CODE[4], sizeof(int32_t));
                uint64_t fail_path_addr = static_cast<uint64_t>(InputDataCache::ORIGINAL_CODE_ADDRESS + 8 + jne_rel_offset);

                tp.insert(tp.end(), { 0x85, 0xC0 });
                tp.insert(tp.end(), { 0x74, 0x0E }); 

                tp.insert(tp.end(), { 0xFF, 0x25, 0x00, 0x00, 0x00, 0x00 });
                tp.insert(tp.end(), reinterpret_cast<uint8_t*>(&fail_path_addr), reinterpret_cast<uint8_t*>(&fail_path_addr) + 8);

                tp.push_back(0x50);
                tp.insert(tp.end(), { 0x48, 0x8B, 0x45, 0xF7 });

                size_t disp_pos1 = tp.size() + 3;
                tp.insert(tp.end(), { 0x48, 0x89, 0x05, 0x00, 0x00, 0x00, 0x00 });
                int32_t disp1 = static_cast<int32_t>(static_cast<uint64_t>(InputDataCache::ALLOCATED_DATA_ADDRESS) - (static_cast<uint64_t>(InputDataCache::ALLOCATED_CAVE_ADDRESS) + disp_pos1 + 4));
                std::memcpy(&tp[disp_pos1], &disp1, sizeof(int32_t));

                tp.insert(tp.end(), { 0x48, 0x8B, 0x45, 0xFF });

                size_t disp_pos2 = tp.size() + 3;
                tp.insert(tp.end(), { 0x48, 0x89, 0x05, 0x00, 0x00, 0x00, 0x00 });
                int32_t disp2 = static_cast<int32_t>(static_cast<uint64_t>(InputDataCache::ALLOCATED_DATA_ADDRESS + 8) - (static_cast<uint64_t>(InputDataCache::ALLOCATED_CAVE_ADDRESS) + disp_pos2 + 4));
                std::memcpy(&tp[disp_pos2], &disp2, sizeof(int32_t));

                tp.push_back(0x58);

                tp.insert(tp.end(), InputDataCache::ORIGINAL_CODE.begin() + 8, InputDataCache::ORIGINAL_CODE.end());

                tp.insert(tp.end(), { 0xFF, 0x25, 0x00, 0x00, 0x00, 0x00 });
                uint64_t ret_addr = static_cast<uint64_t>(InputDataCache::ORIGINAL_CODE_ADDRESS + InputDataCache::ORIGINAL_CODE_SIZE);
                tp.insert(tp.end(), reinterpret_cast<uint8_t*>(&ret_addr), reinterpret_cast<uint8_t*>(&ret_addr) + 8);

                MemoryUtility::WriteMemoryOrThrow(&process, InputDataCache::ALLOCATED_CAVE_ADDRESS, tp.data(), tp.size());

                std::vector<uint8_t> hook_bytes(InputDataCache::ORIGINAL_CODE_SIZE, 0x90);
                hook_bytes[0] = 0xFF; hook_bytes[1] = 0x25;
                hook_bytes[2] = 0x00; hook_bytes[3] = 0x00; hook_bytes[4] = 0x00; hook_bytes[5] = 0x00;
                std::memcpy(&hook_bytes[6], &InputDataCache::ALLOCATED_CAVE_ADDRESS, 8);

                MemoryUtility::WriteMemoryOrThrow(&process, InputDataCache::ORIGINAL_CODE_ADDRESS, hook_bytes.data(), hook_bytes.size());
                
                InputDataCache::HOOK_INSTALLED = true;
            }
            catch (...)
            {
                // If it failed midway, try to clean up the bytes
                MemoryUtility::TryWriteMemoryOrNothing(&process, InputDataCache::ORIGINAL_CODE_ADDRESS, InputDataCache::ORIGINAL_CODE.data(), InputDataCache::ORIGINAL_CODE_SIZE);
                InputDataCache::HOOK_INSTALLED = false;
                throw;
            }
        }

        return static_cast<uintptr_t>(InputDataCache::ALLOCATED_DATA_ADDRESS);
    }
    ...
    {
        std::scoped_lock lock(RacerCache::MUTEX, CameraCache::MUTEX, LapTimeCache::MUTEX, ProgressCache::MUTEX, InputDataCache::MUTEX);
        try 
        {
            ...
            if (InputDataCache::HOOK_INSTALLED && InputDataCache::ORIGINAL_CODE_ADDRESS != INVALID_ADDRESS)
            {
                MemoryUtility::TryWriteMemoryOrNothing(&process, InputDataCache::ORIGINAL_CODE_ADDRESS, InputDataCache::ORIGINAL_CODE.data(), InputDataCache::ORIGINAL_CODE_SIZE);
            }
            FreeIfRequired(InputDataCache::ALLOCATED_CAVE_ADDRESS, InputDataCache::ALLOCATED_CAVE_SIZE);
            FreeIfRequired(InputDataCache::ALLOCATED_DATA_ADDRESS, InputDataCache::ALLOCATED_DATA_SIZE);
        }  
        ...
        InputDataCache::ORIGINAL_CODE_ADDRESS         = INVALID_ADDRESS;
        InputDataCache::ORIGINAL_CODE_CACHE_HAS_VALUE = false;
        InputDataCache::ALLOCATED_CAVE_ADDRESS        = INVALID_ADDRESS;
        InputDataCache::ALLOCATED_DATA_ADDRESS        = INVALID_ADDRESS;
        InputDataCache::HOOK_INSTALLED                = false;
    }
}
