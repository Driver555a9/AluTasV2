...
#ifdef COMMENT_OUT_FILE_ENTIRE_MEMORY_RW
...
namespace AsphaltTas
{
    ...
    RacerState MemoryRW::ReadRacerState()
    {
        ...
        std::memcpy(glm::value_ptr(trans),    &buffer[RacerStateAddresses::OFFSET_TRANS_MATRIX], sizeof(decltype(trans)));
    ...
    InputState MemoryRW::ReadInputState()
    {
        if (! InputDataStateAddresses::AddressesAreValid())
            throw MemoryUtility::MemoryManipFailedException("ReadInputState(): Requires valid InputDataStateAddresses::AddressesAreValid() to read Input State");

        const libmem::Process process = MemoryUtility::GetAsphaltProcessOrThrow();

        InputState state_out;
        MemoryUtility::ReadMemoryOrThrow(&process, InputDataStateAddresses::GetPacketIDAddress(), &state_out, sizeof(decltype(state_out)));
        return state_out;
    }
...
//////////////////////////////////////////////////////////
    ...
    struct GameDeterminismCache
    {
        static inline std::atomic<bool> GAME_IS_DETERMINISTIC = false;
        static inline std::mutex GAME_DETERMINISM_MUTEX;

        static inline libmem::Address ORIGINAL_CODE_ADDRESS = INVALID_ADDRESS;
        
        constexpr static size_t ORIGINAL_CODE_SIZE = 15; 
        static inline std::array<uint8_t, ORIGINAL_CODE_SIZE> ORIGINAL_CODE;
        static inline bool ORIGINAL_CODE_CACHE_HAS_VALUE = false;

        constexpr static size_t ALLOCATED_CAVE_SIZE = 0x1000;
        static inline libmem::Address ALLOCATED_CAVE_ADDRESS = INVALID_ADDRESS;

        constexpr static size_t DT_VALUE_SIZE = sizeof(std::uint64_t);
        static inline libmem::Address DT_VALUE_ADDRESS = INVALID_ADDRESS;
    };

    void MemoryRW::MakeGameDeterministic(float target_fps)
    {
        std::scoped_lock lock(GameDeterminismCache::GAME_DETERMINISM_MUTEX);

        std::uint64_t frame_interval_time_micros = static_cast<std::uint64_t>((1.0f / target_fps) * 1e6f);
        auto [process, module] = MemoryUtility::GetAsphaltProcessAndModuleOrThrow();

        if (GameDeterminismCache::GAME_IS_DETERMINISTIC.load())
        {
            if (GameDeterminismCache::DT_VALUE_ADDRESS != INVALID_ADDRESS)
            {
                MemoryUtility::WriteMemoryOrThrow(&process, GameDeterminismCache::DT_VALUE_ADDRESS, &frame_interval_time_micros, sizeof(frame_interval_time_micros));
            }
            return;
        }

        if (GameDeterminismCache::ORIGINAL_CODE_ADDRESS == INVALID_ADDRESS)
        {
        /* 
            48 89 ?? 24 ??    - mov [rsp+??], reg (rbx)
            48 89 ?? 24 ??    - mov [rsp+??], reg (rsi)
            48 89 ?? 24 ??    - mov [rsp+??], reg (rdi)
            55                - push rbp
            41 56             - push r14
            41 57             - push r15
        ...
        */
            GameDeterminismCache::ORIGINAL_CODE_ADDRESS = MemoryUtility::AOBScanOrThrow(
                &process, 
                "48 89 ?? 24 ?? 48 89 ?? 24 ?? 48 89 ?? 24 ?? 55 41 56 41 57 48 8B EC 48 83 EC ?? 48 8D 05 ?? ?? ?? ??", 
                module.base, 
                module.size
            );
        }

        if (!GameDeterminismCache::ORIGINAL_CODE_CACHE_HAS_VALUE)
        {
            MemoryUtility::ReadMemoryOrThrow(&process, GameDeterminismCache::ORIGINAL_CODE_ADDRESS, GameDeterminismCache::ORIGINAL_CODE.data(), 15);
            GameDeterminismCache::ORIGINAL_CODE_CACHE_HAS_VALUE = true;
        }

        if (GameDeterminismCache::ALLOCATED_CAVE_ADDRESS == INVALID_ADDRESS)
            GameDeterminismCache::ALLOCATED_CAVE_ADDRESS = MemoryUtility::AllocMemoryOrThrow(&process, GameDeterminismCache::ALLOCATED_CAVE_SIZE, libmem::Prot::XRW);

        if (GameDeterminismCache::DT_VALUE_ADDRESS == INVALID_ADDRESS)
            GameDeterminismCache::DT_VALUE_ADDRESS = MemoryUtility::AllocMemoryOrThrow(&process, sizeof(uint64_t), libmem::Prot::RW);

        MemoryUtility::WriteMemoryOrThrow(&process, GameDeterminismCache::DT_VALUE_ADDRESS, &frame_interval_time_micros, sizeof(uint64_t));
        ...
        try 
        {
            ...
            // Stolen Bytes
            tp.insert(tp.end(), GameDeterminismCache::ORIGINAL_CODE.begin(), GameDeterminismCache::ORIGINAL_CODE.begin() + 15);

            // mov rax, [DT_VALUE_ADDRESS]
            size_t disp_pos = tp.size() + 3;
            tp.insert(tp.end(), { 0x48, 0x8B, 0x05, 0x00, 0x00, 0x00, 0x00 });
            int32_t disp = static_cast<int32_t>(static_cast<uint64_t>(GameDeterminismCache::DT_VALUE_ADDRESS) - (static_cast<uint64_t>(GameDeterminismCache::ALLOCATED_CAVE_ADDRESS) + disp_pos + 4));
            std::memcpy(&tp[disp_pos], &disp, sizeof(int32_t));

            // mov [r8], rax
            tp.insert(tp.end(), { 0x49, 0x89, 0x00 });

            // Jump back
            tp.insert(tp.end(), { 0xFF, 0x25, 0x00, 0x00, 0x00, 0x00 });
            uint64_t ret_addr = static_cast<uint64_t>(GameDeterminismCache::ORIGINAL_CODE_ADDRESS + 15);
            tp.insert(tp.end(), reinterpret_cast<uint8_t*>(&ret_addr), reinterpret_cast<uint8_t*>(&ret_addr) + 8);

            MemoryUtility::WriteMemoryOrThrow(&process, GameDeterminismCache::ALLOCATED_CAVE_ADDRESS, tp.data(), tp.size());

            //  Write Hook (14-byte jump + 1 NOP)
            std::vector<uint8_t> hook_bytes(15, 0x90); 
            hook_bytes[0] = 0xFF; hook_bytes[1] = 0x25;
            std::memset(&hook_bytes[2], 0, 4);
            std::memcpy(&hook_bytes[6], &GameDeterminismCache::ALLOCATED_CAVE_ADDRESS, 8);

            MemoryUtility::WriteMemoryOrThrow(&process, GameDeterminismCache::ORIGINAL_CODE_ADDRESS, hook_bytes.data(), hook_bytes.size());
            
            GameDeterminismCache::GAME_IS_DETERMINISTIC.store(true);
        }
        catch (...)
        {
            MemoryUtility::TryWriteMemoryOrNothing(&process, GameDeterminismCache::ORIGINAL_CODE_ADDRESS, GameDeterminismCache::ORIGINAL_CODE.data(), GameDeterminismCache::ORIGINAL_CODE_SIZE);
            throw;
        }
    }

    void MemoryRW::RestoreGameNonDeterministic()
    {
        std::scoped_lock lock(GameDeterminismCache::GAME_DETERMINISM_MUTEX);

        if (GameDeterminismCache::GAME_IS_DETERMINISTIC.load())
        {
            auto process = MemoryUtility::GetAsphaltProcessOrThrow();
            if (GameDeterminismCache::ORIGINAL_CODE_ADDRESS != INVALID_ADDRESS && GameDeterminismCache::ORIGINAL_CODE_CACHE_HAS_VALUE)
            {
                MemoryUtility::WriteMemoryOrThrow(&process, GameDeterminismCache::ORIGINAL_CODE_ADDRESS, GameDeterminismCache::ORIGINAL_CODE.data(), GameDeterminismCache::ORIGINAL_CODE_SIZE);
            }
            GameDeterminismCache::GAME_IS_DETERMINISTIC.store(false);
        }
    }

    bool MemoryRW::GameIsDeterministic() noexcept
    {
        return GameDeterminismCache::GAME_IS_DETERMINISTIC.load(std::memory_order::acquire);
    }

    void MemoryRW::InvalidateCache() noexcept
    {
        std::scoped_lock lock(CamCache::DESTROY_RESTORE_CAMERA_MUTEX, GameDeterminismCache::GAME_DETERMINISM_MUTEX);

        //////////////////////////////////////////////////////////
        // Camera Cleanup
        //////////////////////////////////////////////////////////
        if (CamCache::CAMERA_CODE_IS_DESTROYED.load(std::memory_order::acquire))
        {
            try 
            {
                //RestoreCameraUpdateCode();
            } 
            catch (...) {}
        }
        ...
        try 
        {
            ...
            if (GameDeterminismCache::GAME_IS_DETERMINISTIC.load(std::memory_order::acquire))
            {
                //RestoreGameNonDeterministic();
            }

            if (GameDeterminismCache::ALLOCATED_CAVE_ADDRESS != INVALID_ADDRESS)
            {
                MemoryUtility::TryFreeMemoryOrNothing(&process, GameDeterminismCache::ALLOCATED_CAVE_ADDRESS, GameDeterminismCache::ALLOCATED_CAVE_SIZE);
            }

            if (GameDeterminismCache::DT_VALUE_ADDRESS != INVALID_ADDRESS)
            {
                MemoryUtility::TryFreeMemoryOrNothing(&process, GameDeterminismCache::DT_VALUE_ADDRESS, GameDeterminismCache::DT_VALUE_SIZE);
            }
        } 
        catch (...) 
        {
        ...
        GameDeterminismCache::GAME_IS_DETERMINISTIC.store(false, std::memory_order::release);
        GameDeterminismCache::ORIGINAL_CODE_ADDRESS         = INVALID_ADDRESS;
        GameDeterminismCache::ORIGINAL_CODE_CACHE_HAS_VALUE = false;
        GameDeterminismCache::ALLOCATED_CAVE_ADDRESS        = INVALID_ADDRESS;
        GameDeterminismCache::DT_VALUE_ADDRESS              = INVALID_ADDRESS;
    }
}
