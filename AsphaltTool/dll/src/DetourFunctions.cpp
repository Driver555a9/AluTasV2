    ...
    namespace DetourFunctions 
    {
            ...
            namespace
            {
                ...
                uintptr_t* Detour_NewPhysicsFrameFunction(void* p_this, uintptr_t* p_out_delta_micros, uintptr_t* p_in_delta_micros)
                {
                    ...
                    if (g_ticks_left_to_skip > 0)
                    {
                        ...
                        // Nocall since nitro activation logic may happen with 0 dt
                        //return RealNewPhysicsFrameCall(p_this, p_out_delta_micros, p_in_delta_micros);
                        *p_in_delta_micros  = 0;
        ...
        namespace NewBulletPhysicsTick
        {
            namespace
            {
                ...
                {
                    //const uintptr_t* ptr_0x160 = reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(p_this) + 0x160);
                    //const uintptr_t* ptr_0x168 = reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(p_this) + 0x168);
                    //const bool ptr_1_is_zero = *ptr_0x160 == 0;
                    //const bool ptr_2_is_zero = *ptr_0x168 == 0;
                    {
                        LOCK_CURRENT_STATE_MUTEX();
                        //GameDLLState::g_current_state.m_meta_data.m_is_in_race = (!ptr_1_is_zero && !ptr_2_is_zero);
                    }
                ...
                {
                    {
                        ...
                        // Brake is called first (before steer & new frame physics logic thread)
