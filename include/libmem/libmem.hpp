...
namespace libmem {
	...
	{
		T dest;
		bool b = ReadMemory(source, reinterpret_cast<uint8_t *>(&dest), sizeof(dest));
		return dest;
	}
	...
	/// Reads memory from a source address in a remote process
	template <typename T>
    [[nodiscard]] std::optional<T> ReadMemory(const Process *process, Address source)
	{
		T dest;
	...
	{
		bool b = WriteMemory(dest, reinterpret_cast<uint8_t *>(&source), sizeof(T));
	}
	...
	{
		bool b = WriteMemory(process, dest, reinterpret_cast<uint8_t *>(&source), sizeof(T));
	}
	...
	/// Resolves a deep pointer (also known as pointer scan or pointer map) in the current process
	template <typename T>
	[[nodiscard]] T *DeepPointer(Address base, const std::vector<Address> &offsets)
	{
