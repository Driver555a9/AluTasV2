...
namespace AsphaltTas
{
    ...
    RacerState MemoryRW::ReadRacerState()
    {
        ...
        constexpr size_t RACER_BLOCK_SIZE = ( RacerStateAddresses::OFFSET_VELOCITY_VEC3 + sizeof(decltype(velocity)) ) - RacerStateAddresses::OFFSET_TRANS_MATRIX4x4;

        const auto [min_addr, max_addr] = RacerStateAddresses::GetMinMaxDataAddressSpace();

        std::array<std::byte, RACER_BLOCK_SIZE> buffer;

        MemoryUtility::ReadMemoryOrThrow(&process, min_addr, buffer.data(), sizeof(buffer));
    ...
    CameraState MemoryRW::ReadCameraState()
    {
        ...
        const auto [min_addr, max_addr] = CameraStateAddresses::GetMinMaxDataAddressSpace();
        ...
        //////////////////////////////////////////////////////////
        // Swapping Y and Z to convert to XYZW convention; Flipping sign of z for handiness
        //////////////////////////////////////////////////////////
    ...
    void MemoryRW::WriteRacerState(const RacerState& state)
    {
        ...
        MemoryUtility::WriteGlmValueOrThrow(&process, RacerStateAddresses::GetTransMatrixAddress(),  state.GetTransformMatrix());
    ...
    void MemoryRW::WriteCameraState(const CameraState& state, IGNORE_FLAG_CAMERA ignore_flags)
    {
        ...
        copy.m_position.y *= -1.0f;

        
        //////////////////////////////////////////////////////////
        // Swapping Y and Z to convert to XZYW convention; Flipping sign of (y) = Z in glm convention for handiness
        //////////////////////////////////////////////////////////
        std::swap(copy.m_rotation.y, copy.m_rotation.z);
        copy.m_rotation.y *= -1.0f;
